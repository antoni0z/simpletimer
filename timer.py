...
class DeepWorkTimerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self._refresh_stats()
        self._schedule_day_change_check()
    ...
    def _schedule_day_change_check(self) -> None:
        self.day_change_after_id = self.root.after(
            DAY_CHANGE_CHECK_MS, self._check_for_day_change
        )

    def _cancel_day_change_check(self) -> None:
        if self.day_change_after_id is None:
            return

        try:
            self.root.after_cancel(self.day_change_after_id)
        except tk.TclError:
            pass
        finally:
            self.day_change_after_id = None

    def _check_for_day_change(self) -> None:
        self.day_change_after_id = None

        if self.stats_date != datetime.now().date():
            self._refresh_stats()
